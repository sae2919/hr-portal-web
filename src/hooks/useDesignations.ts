...
export const useDesignations = (params?: {
  ...
  per_page?: number; // pass 100 for dropdowns, omit for paginated tables
}) => {
  return useQuery({
    queryKey: [KEY, params],
    queryFn: async () => {
...
export const useCreateDesignation = () => {
  ...
  return useMutation({
    mutationFn: async (data: StoreDesignationPayload) => {
...
export const useUpdateDesignation = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...data }: UpdateDesignationPayload) => {
